...
function OnboardingFirstItem({ onComplete }: { onComplete: (firstItem?: Item) => void }) {
  ...
  const addItem = () => {
    if (!name.trim()) return;
    onComplete({ name: name.trim(), emoji, amount, remaining, due: 'Friday', confidence: 100, cadence: 'Usually every 7 days', status: 'Soon', selected: true });
  };
...
function OnboardingFirstItemChooser({ onComplete }: { onComplete: (firstItem?: Item) => void }) {
  ...
  const [remaining, setRemaining] = useState('About half left');
  const commonEmojis = ['🥛', '🥚', '🍞', '☕', '🍎', '🥦', '🧀', '🫒'];
  const addManualItem = () => { if (name.trim()) onComplete({ name: name.trim(), emoji, amount, remaining, due: 'Friday', confidence: 100, cadence: 'Usually every 7 days', status: 'Soon', selected: true }); };

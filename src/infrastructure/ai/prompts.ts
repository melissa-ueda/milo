import { CATEGORY_CODES } from "@/src/domain/taxonomy/category";
import { PRODUCT_TYPE_CODES } from "@/src/domain/taxonomy/product-type";

/**
 * Prompt templates. The allowed `type`/`category` values are interpolated from
 * the single domain code lists — the same arrays that back the domain types and
 * (loosely) the zod schemas. One source of vocabulary, no triplication.
 */
const TYPE_LIST = PRODUCT_TYPE_CODES.join(", ");
const CATEGORY_LIST = CATEGORY_CODES.join(", ");
...
- type MUST be exactly one of these codes: ${TYPE_LIST}
- Never invent new type codes; if nothing fits, use "other"
- category MUST be exactly one of these codes: ${CATEGORY_LIST}
- Never invent new category codes; if nothing fits, use "other"
- quantity and unit should reflect what was purchased (e.g. quantity: 2, unit: "L" for 2 liters)
- price is the total price for that line item in the receipt currency, as a number
...
Use the household profile and purchase history to predict when each known product will likely run out and whether it belongs on the next shopping list.

Rules:
- Return exactly one prediction for every product provided, keyed by its normalizedName.

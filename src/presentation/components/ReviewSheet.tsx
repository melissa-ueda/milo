...
import { CATEGORY_CODES, type Category } from "@/src/domain/taxonomy/category";
import {
  PRODUCT_TYPE_CODES,
  type ProductType,
} from "@/src/domain/taxonomy/product-type";
import { emojiForType } from "@/src/domain/taxonomy/emoji";
import { fromMajor, sumMoney, toMajor } from "@/src/domain/values/money";
import { categoryLabel } from "../labels/en";
import { formatMoney } from "../labels/format";

type ReviewSheetProps = {
  receipt: ParsedReceipt;
  items: ReviewLine[];
  onItemsChange: (items: ReviewLine[]) => void;
  onSave: () => void;
  onClose: () => void;
  saving?: boolean;
};
...
export function ReviewSheet({
  ...
  const updateItem = (id: string, updates: Partial<ReviewLine>) =>
    onItemsChange(
      items.map((item) => (item.id === id ? { ...item, ...updates } : item)),
    );

  const total = formatMoney(sumMoney(items.map((item) => item.price)));
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-2">
            {items.map((item) =>
              editingId === item.id ? (
                ...
                >
                  <span className="text-2xl">{emojiForType(item.type)}</span>
...
function EditItemRow({
  ...
  item: ReviewLine;
  onSave: (updates: Partial<ReviewLine>) => void;
  onCancel: () => void;
}) {
  const [name, setName] = useState(item.normalizedName);
  const [type, setType] = useState<ProductType>(item.type);
  const [category, setCategory] = useState<Category>(item.category);
  const [quantity, setQuantity] = useState(String(item.quantity));
  const [unit, setUnit] = useState(item.unit);
  const [price, setPrice] = useState(String(toMajor(item.price)));
  ...
  const handleSave = () => {
    onSave({
      ...
      unit: unit.trim(),
      price: fromMajor(parseFloat(price) || 0, item.price.currency),
    });
  };
  ...
  return (
    <div className="rounded-xl border border-[#28704c] bg-[#eef5eb] p-4 space-y-3">
      ...
      >
        {PRODUCT_TYPE_CODES.map((value) => (
          <option key={value} value={value}>
            {emojiForType(value)} {value.replace(/_/g, " ")}
          </option>
        ))}
      </select>
      <select
